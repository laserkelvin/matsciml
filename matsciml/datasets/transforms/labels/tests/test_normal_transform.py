...
from matsciml.datasets.transforms.labels.normal import NormalLabelTransform
...
def test_transform_workflow(dset_cls_name: str, label_key: str):
    ...
    dset = dset_class.from_devset(
        transforms=[
            NormalLabelTransform(label_key, mean=0.0, std=1.0, agg_method="static")
        ]
    )
...
def test_transform_dynamic_sampled(dset_cls_name: str, label_key: str):
    dset_class = registry.get_dataset_class(dset_cls_name)
    dset = dset_class.from_devset(
        transforms=[
            NormalLabelTransform(label_key, agg_method="sampled", num_samples=10)
        ]
    )
    assert dset.transforms[0].mean
